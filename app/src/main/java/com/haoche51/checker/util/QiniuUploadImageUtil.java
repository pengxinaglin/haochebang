...
public class QiniuUploadImageUtil implements HCHttpCallback {
  /**
   * 获取待上传图片数量
   */
  private final String IMAGTAG = "QiniuUploadImageUtil";
  ...
  public QiniuUploadImageUtil(Activity mActivity, List<String> images) {
...
//			Recorder recorder = new FileRecorder(recorderPath);
  ...
  private void compressImages(PhotoEntity image, int position) {
    if (image.getPath() != null && (image.getUrl() == null || TextUtils.isEmpty(image.getUrl()))) { // 路径存在，且未上传过
      Map<String, Object> map = BitmapUtil.readCorrectBitMapFromFile(image.getPath(), BitmapUtil.ResolutionLevel.kResolutionMax);
    ...
    public void complete(String arg0, ResponseInfo arg1, JSONObject arg2) {
      if (arg1.isOK()) {
        ...
        if (arg1.isServerError()) {
          ...
          uploadFailed(PictureConstants.UPLOAD_UNKNOW_ERROR); //未知异常错误
        }
      }
      // 如果上传数 == 上传总数 认为上传完成
