...
public class ScrollOverListView extends ListView {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		if (ev.getAction() == MotionEvent.ACTION_DOWN) {
			if (DEBUG) HCLogUtil.d(TAG, "onInterceptTouchEvent Action down");
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		switch(action){
			case MotionEvent.ACTION_DOWN:{
				if (DEBUG) HCLogUtil.d(TAG, "action down");
				mLastY = y;
			...
			case MotionEvent.ACTION_MOVE:{
				if (DEBUG) HCLogUtil.d(TAG, "action move");
				...
				if (DEBUG) HCLogUtil.d(TAG, "lastY=" + mLastY + " y=" + y);
				...
				if (firstVisiblePosition <= mTopPosition && firstTop >= listPadding && deltaY > 0) {
					if (DEBUG) HCLogUtil.d(TAG, "action move pull down");
		        ...
		        if (firstVisiblePosition + childCount >= itemCount && lastBottom <= end && deltaY < 0) {
							if (DEBUG) HCLogUtil.d(TAG, "action move pull up");
