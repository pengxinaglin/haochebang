...
/**
 * 待分配列表
 * Created by wufx on 16/05/13.
 */
public class PurchaseAssignedFragment extends CommonBaseFragment implements AdapterView.OnItemClickListener, View.OnClickListener, HCPullToRefresh.OnRefreshCallback {
    ...
    DataObserver mDataObserver = new DataObserver(new Handler()) {
        ...
        public void onChanged(Bundle data) {
            try {
                if (data != null) {
                    //添加任务
                    if (data.getBoolean(TaskConstants.BINDLE_ADD_ITEM)) {
                    ...
                    //分配 移除item
                    if (data.getBoolean(TaskConstants.BINDLE_IS_ASSIGN) && data.getBoolean(TaskConstants.BINDLE_TRANSFER_TO_OTHER)) {
...
//            List<PurchaseTaskShortEntity> tempList = new HCJsonParse().parsePurchaseShortEntitys(json);
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
        ...
        intent.putExtra(TaskConstants.BINDLE_IS_ASSIGN, true);
...
//            showLoadingView(false);
    ...
    public void onPullDownRefresh() {
        pageIndex = 0;
        OKHttpManager.getInstance().post(HCHttpRequestParam.getBackTaskList(pageIndex, TaskConstants.REQUEST_PURCHASE_ASSIGNED, TaskConstants.DEFAULT_SHOWTASK_COUNT,search), this, HttpConstants.GET_LIST_REFRESH);//下拉刷新
    }

    @Override
    public void onLoadMoreRefresh() {
        OKHttpManager.getInstance().post(HCHttpRequestParam.getBackTaskList(++pageIndex, TaskConstants.REQUEST_PURCHASE_ASSIGNED, TaskConstants.DEFAULT_SHOWTASK_COUNT,search), this, HttpConstants.GET_LIST_LOADMORE);//上拉刷新
    }
