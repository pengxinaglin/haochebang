 ...
 */
public class OfflineSoldSaleFragment extends CommonBaseFragment implements AdapterView.OnItemClickListener, View.OnClickListener, HCPullToRefresh.OnRefreshCallback {

  private HCPullToRefresh mPullToRefresh;
  private ListView mListView;
  private List<OfflineShortEntity> mTasks = new ArrayList<OfflineShortEntity>();
  ...
  public static String BINDLE_CONFIRM_TRANS = "bindle_confirm_trans";
  ...
  protected void initData(Bundle savedInstanceState) {
    ...
    //读取ACache中的缓存记录
    String json = mAcache.getAsString(TaskConstants.ACACHE_OFFLINESOLD_SALE);
...
//      List<OfflineShortEntity> tempList = new HCJsonParse().parseOfflineShortEntitys(json);
    ...
    if (mAdapter == null) {
      mAdapter = new OfflineSoldStockAdapter(getActivity(), mTasks, R.layout.layout_offlinesold_stock_item, TaskConstants.REQUEST_OFFLINESOLD_SALE);
    }
    //设置数据
    if (mListView != null) {
    ...
    public void onChanged(Bundle data) {
      try {
        if (data != null) {
          ...
          // 更新
          if (data.getBoolean(BINDLE_CONFIRM_TRANS)) {
            OfflineShortEntity entity = mTasks.get(mClickIndex);
            entity.setStatus_text("售出待确认收款");
          }

          if (mTasks.size() > TaskConstants.DEFAULT_SHOWTASK_COUNT)
//            mAcache.put(TaskConstants.ACACHE_OFFLINESOLD_SALE, new HCJsonParse().parseOffLineSoldTaskListToJson(mTasks.subList(0, TaskConstants.DEFAULT_SHOWTASK_COUNT)));//缓存删除过本条数据后的json
            mAcache.put(TaskConstants.ACACHE_OFFLINESOLD_SALE, JsonParseUtil.toJsonArray(mTasks.subList(0, TaskConstants.DEFAULT_SHOWTASK_COUNT)));//缓存删除过本条数据后的json
          else
//            mAcache.put(TaskConstants.ACACHE_OFFLINESOLD_SALE, new HCJsonParse().parseOffLineSoldTaskListToJson(mTasks));//缓存删除过本条数据后的json
            mAcache.put(TaskConstants.ACACHE_OFFLINESOLD_SALE, JsonParseUtil.toJsonArray(mTasks));//缓存删除过本条数据后的json
...
//          List<OfflineShortEntity> viewTaskList = new HCJsonParse().parseOfflineShortEntitys(response.getData());
          ...
          if (requestId == HttpConstants.GET_LIST_REFRESH) {
            if (response.getData() != null)
              mAcache.put(TaskConstants.ACACHE_OFFLINESOLD_SALE, response.getData());//只缓存刷新的数据
            mTasks.clear();
...
//      showLoadingView(false);
  ...
  public void onPullDownRefresh() {
    pageIndex = 0;
    OKHttpManager.getInstance().post(HCHttpRequestParam.getBackStockList(pageIndex, TaskConstants.REQUEST_OFFLINESOLD_SALE, TaskConstants.DEFAULT_SHOWTASK_COUNT, search), this, HttpConstants.GET_LIST_REFRESH);//下拉刷新
  }

  @Override
  public void onLoadMoreRefresh() {
    OKHttpManager.getInstance().post(HCHttpRequestParam.getBackStockList(++pageIndex, TaskConstants.REQUEST_OFFLINESOLD_SALE, TaskConstants.DEFAULT_SHOWTASK_COUNT, search), this, HttpConstants.GET_LIST_LOADMORE);//上拉刷新
  }
}