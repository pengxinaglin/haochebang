...
import com.haoche51.checker.adapter.PurchaseCompleteAdapter;
 ...
 */
public class PurchaseCompleteFragment extends CommonBaseFragment implements AdapterView.OnItemClickListener, View.OnClickListener, HCPullToRefresh.OnRefreshCallback {

    private HCPullToRefresh mPullToRefresh;
    private ListView mListView;
    private List<PurchaseTaskShortEntity> mTasks = new ArrayList<>();
    private PurchaseCompleteAdapter mAdapter = null;
        ...
        public void onChanged(Bundle data) {
            try {
                if (data != null) {
                    ...
                    //复活任务或者毁约都需要删除当前记录
                    if (data.getBoolean(TaskConstants.BINDLE_BACK_LIFE) || data.getBoolean(TaskConstants.BINDLE_BREAK_CONTRACT)) {
                        mTasks.remove(mClickIndex);
                    }

                    if (mTasks.size() > TaskConstants.DEFAULT_SHOWTASK_COUNT)
//                        mAcache.put(TaskConstants.ACACHE_PURCHASE_COMPLETE, new HCJsonParse().parsePurchasekTaskListToJson(mTasks.subList(0, TaskConstants.DEFAULT_SHOWTASK_COUNT)));//缓存删除过本条数据后的json
                        mAcache.put(TaskConstants.ACACHE_PURCHASE_COMPLETE, JsonParseUtil.toJsonArray(mTasks.subList(0, TaskConstants.DEFAULT_SHOWTASK_COUNT)));//缓存删除过本条数据后的json
                    else
//                        mAcache.put(TaskConstants.ACACHE_PURCHASE_COMPLETE, new HCJsonParse().parsePurchasekTaskListToJson(mTasks));//缓存删除过本条数据后的json
                        mAcache.put(TaskConstants.ACACHE_PURCHASE_COMPLETE, JsonParseUtil.toJsonArray(mTasks));//缓存删除过本条数据后的json
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        //读取ACache中的缓存记录
        String json = mAcache.getAsString(TaskConstants.ACACHE_PURCHASE_COMPLETE);
...
//            List<PurchaseTaskShortEntity> tempList = new HCJsonParse().parsePurchaseShortEntitys(json);
        ...
        if (mAdapter == null) {
            mAdapter = new PurchaseCompleteAdapter(getActivity(), mTasks, R.layout.layout_purchase_buyback_item);
        }
        //设置数据
        if (mListView != null) {
...
//                    List<PurchaseTaskShortEntity> viewTaskList = new HCJsonParse().parsePurchaseShortEntitys(response.getData());
                    ...
                    if (requestId == HttpConstants.GET_LIST_REFRESH) {
                        if (response.getData() != null)
                            mAcache.put(TaskConstants.ACACHE_PURCHASE_COMPLETE, response.getData());//只缓存刷新的数据
                        mTasks.clear();
...
//            showLoadingView(false);
    ...
    public void onPullDownRefresh() {
        pageIndex = 0;
        OKHttpManager.getInstance().post(HCHttpRequestParam.getBackTaskList(pageIndex, TaskConstants.REQUEST_PURCHASE_COMPLETE, TaskConstants.DEFAULT_SHOWTASK_COUNT,search), this, HttpConstants.GET_LIST_REFRESH);//下拉刷新
    }


    @Override
    public void onLoadMoreRefresh() {
        OKHttpManager.getInstance().post(HCHttpRequestParam.getBackTaskList(++pageIndex, TaskConstants.REQUEST_PURCHASE_COMPLETE, TaskConstants.DEFAULT_SHOWTASK_COUNT,search), this, HttpConstants.GET_LIST_LOADMORE);//上拉刷新
    }
