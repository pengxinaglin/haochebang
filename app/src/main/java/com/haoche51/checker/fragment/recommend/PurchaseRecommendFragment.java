...
import com.haoche51.checker.adapter.PurchaseCluesAdapter;
...
import static com.haoche51.checker.activity.recommend.RecommendMainActivity.REQUEST_ADD_PURCHASE_CLUE;
...
public class PurchaseRecommendFragment extends CommonBaseFragment implements HCPullToRefresh.OnRefreshCallback, View.OnClickListener {
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        if (!TextUtils.isEmpty(json)) {
            this.allLeads.clear();
//			this.allLeads.addAll(new HCJsonParse().parseGetPurchaseCluesEntitys(json));
            List<PurchaseCluesEntity> tempList = JsonParseUtil.fromJsonArray(json, PurchaseCluesEntity.class);
            if (tempList != null) {
...
//                    List<PurchaseCluesEntity> tempList = new HCJsonParse().parseGetPurchaseCluesEntitys(obj.getString("list"));
                    List<PurchaseCluesEntity> tempList = JsonParseUtil.fromJsonArray(obj.getString("list"), PurchaseCluesEntity.class);
    ...
    public void onPullDownRefresh() {
        pageIndex = 0;
        OKHttpManager.getInstance().post(HCHttpRequestParam.getPurchaseleadList(pageIndex,
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_ADD_PURCHASE_CLUE) {
            // 重新刷新数据
