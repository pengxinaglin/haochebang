...
import com.haoche51.checker.adapter.DealApproveAdapter;
 ...
 */
public class DealApproveFragment extends CommonBaseFragment implements HCPullToRefresh.OnRefreshCallback, AdapterView.OnItemClickListener, View.OnClickListener {
    ...
    private List<DealApproveEntity> mTasks = new ArrayList<>();
    private DealApproveAdapter mAdapter = null;
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        //读取ACache中的缓存记录
        String json = mAcache.getAsString(TaskConstants.ACACHE_DEAL_APPROVE);
        if (!TextUtils.isEmpty(json)) {
            this.mTasks.clear();
//            List<DealApproveEntity> tempList = new HCJsonParse().parseC2CTransToList(json);
            List<DealApproveEntity> tempList = JsonParseUtil.fromJsonArray(json, DealApproveEntity.class);
            if (tempList != null)
                this.mTasks.addAll(tempList);
        }
        if (mAdapter == null) {
            mAdapter = new DealApproveAdapter(getActivity(), mTasks, R.layout.item_repurchase_approve);
        }
        //设置数据
        if (mListView != null) {
...
//            dismissLoadingView();
            switch (response.getErrno()) {
                case 0:
//                    List<DealApproveEntity> viewTaskList = new HCJsonParse().parseC2CTransToList(response.getData());
                    List<DealApproveEntity> viewTaskList = JsonParseUtil.fromJsonArray(response.getData(), DealApproveEntity.class);
                    //刷新
                    if (requestId == HttpConstants.GET_LIST_REFRESH) {
                        if (response.getData() != null)
                            mAcache.put(TaskConstants.ACACHE_DEAL_APPROVE, response.getData());//只缓存刷新的数据
                        mTasks.clear();
    ...
    public void onHttpComplete(String action, int requestId, HCHttpResponse response, Throwable error) {
        if (action.equals(HttpConstants.ACTION_GET_DEAL_LIST)) {
    ...
    public void onPullDownRefresh() {
        pageIndex = 0;
        OKHttpManager.getInstance().post(HCHttpRequestParam.getDealAproveList(pageIndex, TaskConstants.DEFAULT_SHOWTASK_COUNT, search), this, HttpConstants.GET_LIST_REFRESH);//下拉刷新
    }

    @Override
    public void onLoadMoreRefresh() {
        OKHttpManager.getInstance().post(HCHttpRequestParam.getDealAproveList(++pageIndex, TaskConstants.DEFAULT_SHOWTASK_COUNT, search), this, HttpConstants.GET_LIST_LOADMORE);//上拉刷新
    }
