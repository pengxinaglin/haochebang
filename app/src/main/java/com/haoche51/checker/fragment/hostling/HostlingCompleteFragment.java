 ...
 */
public class HostlingCompleteFragment extends CommonBaseFragment implements AdapterView.OnItemClickListener, View.OnClickListener, HCPullToRefresh.OnRefreshCallback {
        ...
        public void onChanged(Bundle data) {
            try {
                if (data != null) {
                    ...
                    //移除item
                    if (data.getBoolean(TaskConstants.BINDLE_RESORTING_TASK)) {
                        mTasks.remove(mClickIndex);
                    }
                    if (mTasks.size() > TaskConstants.DEFAULT_SHOWTASK_COUNT)
//                        mAcache.put(TaskConstants.ACACHE_HOSTLING_COMPLETE, new HCJsonParse().parseHostlingTaskListToJson(mTasks.subList(0, TaskConstants.DEFAULT_SHOWTASK_COUNT)));//缓存删除过本条数据后的json
                        mAcache.put(TaskConstants.ACACHE_HOSTLING_COMPLETE, JsonParseUtil.toJsonArray(mTasks.subList(0, TaskConstants.DEFAULT_SHOWTASK_COUNT)));//缓存删除过本条数据后的json
                    else
//                        mAcache.put(TaskConstants.ACACHE_HOSTLING_COMPLETE, new HCJsonParse().parseHostlingTaskListToJson(mTasks));//缓存删除过本条数据后的json
                        mAcache.put(TaskConstants.ACACHE_HOSTLING_COMPLETE, JsonParseUtil.toJsonArray(mTasks));//缓存删除过本条数据后的json
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        //读取ACache中的缓存记录
        String json = mAcache.getAsString(TaskConstants.ACACHE_HOSTLING_COMPLETE);
...
//            List<HostlingTaskShortEntity> tempList = new HCJsonParse().parseHostlingTaskShortEntitys(json);
        ...
        if (mAdapter == null) {
            mAdapter = new HostlingTaskAdapter(getActivity(), mTasks, R.layout.layout_hostling_item, TaskConstants.REQUEST_HOSTLING_COMPLETE);
        }
        //设置数据
        if (mListView != null) {
...
//                    List<HostlingTaskShortEntity> viewTaskList = new HCJsonParse().parseHostlingTaskShortEntitys(response.getData());
                    ...
                    if (requestId == HttpConstants.GET_LIST_REFRESH) {
                        if (response.getData() != null)
                            mAcache.put(TaskConstants.ACACHE_HOSTLING_COMPLETE, response.getData());//只缓存刷新的数据
                        mTasks.clear();
...
//            showLoadingView(false);
    ...
    public void onPullDownRefresh() {
        pageIndex = 0;
        OKHttpManager.getInstance().post(HCHttpRequestParam.getBackRepairList(pageIndex, TaskConstants.REQUEST_HOSTLING_COMPLETE, TaskConstants.DEFAULT_SHOWTASK_COUNT, search), this, HttpConstants.GET_LIST_REFRESH);//下拉刷新
    }

    @Override
    public void onLoadMoreRefresh() {
        OKHttpManager.getInstance().post(HCHttpRequestParam.getBackRepairList(++pageIndex, TaskConstants.REQUEST_HOSTLING_COMPLETE, TaskConstants.DEFAULT_SHOWTASK_COUNT,search), this, HttpConstants.GET_LIST_LOADMORE);//上拉刷新
    }
}