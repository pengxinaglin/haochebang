 ...
 */
public class HostlingUnderWayFragment extends CommonBaseFragment implements AdapterView.OnItemClickListener, View.OnClickListener, HCPullToRefresh.OnRefreshCallback {

    public static String BINDLE_PURCHASE_TASK = "bindle_hostling_underway_task", BINDLE_MODIFY_PLATE_UNIX = "bindle_modify_plate_unix";
    ...
    DataObserver mDataObserver = new DataObserver(new Handler()) {
        ...
        public void onChanged(Bundle data) {
            try {
                if (data != null) {
                    if (data.getBoolean(TaskConstants.BINDLE_RESORTING_TASK) || data.getBoolean(TaskConstants.BINDLE_COMMIT_HOSTLING)) {
                    ...
                    //移除item
                    if (data.getBoolean(BINDLE_PURCHASE_TASK)) {
                        mTasks.remove(mClickIndex);
                    } else if (data.getBoolean(BINDLE_MODIFY_PLATE_UNIX)) {
                        HostlingTaskShortEntity entity = mTasks.get(mClickIndex);
                        if (data.getInt("pick_up_time") > 0)
                            entity.setPick_up_time(UnixTimeUtil.formatYearMonthDay(data.getInt("pick_up_time")));
                        if (data.getInt("servicing_fee") > 0)
                            entity.setExpect_price(data.getInt("servicing_fee"));
                    }

                    if (mTasks.size() > TaskConstants.DEFAULT_SHOWTASK_COUNT)
//                        mAcache.put(TaskConstants.ACACHE_HOSTLING_UNDERWAY, new HCJsonParse().parseHostlingTaskListToJson(mTasks.subList(0, TaskConstants.DEFAULT_SHOWTASK_COUNT)));//缓存删除过本条数据后的json
                        mAcache.put(TaskConstants.ACACHE_HOSTLING_UNDERWAY, JsonParseUtil.toJsonArray(mTasks.subList(0, TaskConstants.DEFAULT_SHOWTASK_COUNT)));//缓存删除过本条数据后的json
                    else
//                        mAcache.put(TaskConstants.ACACHE_HOSTLING_UNDERWAY, new HCJsonParse().parseHostlingTaskListToJson(mTasks));//缓存删除过本条数据后的json
                        mAcache.put(TaskConstants.ACACHE_HOSTLING_UNDERWAY, JsonParseUtil.toJsonArray(mTasks));//缓存删除过本条数据后的json
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        //读取ACache中的缓存记录
        String json = mAcache.getAsString(TaskConstants.ACACHE_HOSTLING_UNDERWAY);
...
//            List<HostlingTaskShortEntity> tempList = new HCJsonParse().parseHostlingTaskShortEntitys(json);
        ...
        if (mAdapter == null) {
            mAdapter = new HostlingTaskAdapter(getActivity(), mTasks, R.layout.layout_hostling_item, TaskConstants.REQUEST_HOSTLING_UNDERWAY);
        }
        //设置数据
        if (mListView != null) {
...
//                    List<HostlingTaskShortEntity> viewTaskList = new HCJsonParse().parseHostlingTaskShortEntitys(response.getData());
                    ...
                    if (requestId == HttpConstants.GET_LIST_REFRESH) {
                        if (response.getData() != null)
                            mAcache.put(TaskConstants.ACACHE_HOSTLING_UNDERWAY, response.getData());//只缓存刷新的数据
                        mTasks.clear();
...
//            showLoadingView(false);
    ...
    public void onPullDownRefresh() {
        pageIndex = 0;
        mCall = OKHttpManager.getInstance().post(HCHttpRequestParam.getBackRepairList(pageIndex, TaskConstants.REQUEST_HOSTLING_UNDERWAY, TaskConstants.DEFAULT_SHOWTASK_COUNT, search), this, HttpConstants.GET_LIST_REFRESH);//下拉刷新
    }

    @Override
    public void onLoadMoreRefresh() {
        mCall = OKHttpManager.getInstance().post(HCHttpRequestParam.getBackRepairList(++pageIndex, TaskConstants.REQUEST_HOSTLING_UNDERWAY, TaskConstants.DEFAULT_SHOWTASK_COUNT, search), this, HttpConstants.GET_LIST_LOADMORE);//上拉刷新
    }
