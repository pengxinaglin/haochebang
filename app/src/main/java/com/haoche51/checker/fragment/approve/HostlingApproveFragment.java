 ...
 */
public class HostlingApproveFragment extends CommonBaseFragment implements AdapterView.OnItemClickListener, View.OnClickListener, HCPullToRefresh.OnRefreshCallback {
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        //读取ACache中的缓存记录
        String json = mAcache.getAsString(TaskConstants.ACACHE_HOSTLING_APPROVE);
...
//            List<PurchaseTaskShortEntity> tempList = new HCJsonParse().parsePurchaseShortEntitys(json);
    ...
    public void onHttpComplete(String action, int requestId, HCHttpResponse response, Throwable error) {
        if (action.equals(HttpConstants.ACTION_GET_HOSTING_APPROVE_LIST)) {
...
//                    List<PurchaseTaskShortEntity> viewTaskList = new HCJsonParse().parsePurchaseShortEntitys(response.getData());
                    ...
                    if (requestId == HttpConstants.GET_LIST_REFRESH) {
                        if (response.getData() != null)
                            mAcache.put(TaskConstants.ACACHE_HOSTLING_APPROVE, response.getData());//只缓存刷新的数据
                        mTasks.clear();
    ...
    public void onPullDownRefresh() {
        pageIndex = 0;
        OKHttpManager.getInstance().post(HCHttpRequestParam.getHostlingAproveList(pageIndex, TaskConstants.DEFAULT_SHOWTASK_COUNT), this, HttpConstants.GET_LIST_REFRESH);//下拉刷新
    }

    @Override
    public void onLoadMoreRefresh() {
        OKHttpManager.getInstance().post(HCHttpRequestParam.getHostlingAproveList(++pageIndex, TaskConstants.DEFAULT_SHOWTASK_COUNT), this, HttpConstants.GET_LIST_LOADMORE);//上拉刷新
    }
